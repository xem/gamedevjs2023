...
a.onmousemove = (e) => {
  ...
  y = (e.pageY - a.getBoundingClientRect().top - document.documentElement.scrollTop - document.body.scrollTop)*2;
  //console.log(x,y);
...
var handle_clicks = (e) => {
  ...
  if(screen == 3){
    ...
    if(c.isPointInPath(x, y)){
      ...
      window.open("https://www.twitter.com/intent/tweet?text=" + encodeURIComponent("I made a level for #ChronoRobot!\nPlay the game here: https://xem.github.io/gamedevjs2023\nPlay my level here: https://xem.github.io/gamedevjs2023/src#" + JSON.stringify({hash:level_data.hash.replace(/0*$/,''), pipes:level_data.pipes, balances: level_data.balances}).replace("{","%7b").replace("}","%7d").replace(/"/g,"%22")));
      //}
      //else {
